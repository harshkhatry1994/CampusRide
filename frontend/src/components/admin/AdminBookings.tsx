...
export function AdminBookings({ onCountChange }: { onCountChange?: () => void }) {
  ...
  const doAction = async (id: string, action: string, body = {}) => {
    setActionLoading(action + id);
    try {
      const url = action === "delete" ? `${API_URL}/api/admin/bookings/${id}` : `${API_URL}/api/admin/bookings/${id}/status`;
      ...
      const data = await res.json();
      if (data.success) {
        toast.success(`Booking ${action === "delete" ? "deleted" : "updated"} successfully`);
        fetchBookings();
  ...
  const filtered = Array.isArray(bookings) ? bookings.filter((b) => {
    const customerName = b.riderDetails?.name || b.user?.name || "";
    const bikeName = b.bike?.name || `${b.bike?.brand || ""} ${b.bike?.model || ""}`.trim() || "";
    const bookingId = b.bookingId || "";

    const matchesSearch =
      bookingId.toLowerCase().includes(search.toLowerCase()) ||
      customerName.toLowerCase().includes(search.toLowerCase()) ||
      bikeName.toLowerCase().includes(search.toLowerCase());
          ...
          <Table>
            ...
            <TableBody>
              {loading ? (
                ...
                filtered.map((b) => (
                  <TableRow key={b._id} className="group hover:bg-muted/10 transition-colors border-border/40">
                    <TableCell className="px-8 py-4">
                      <div className="flex flex-col">
                        <span className="font-black text-sm text-primary tracking-tight">#{b.bookingId}</span>
                        <span className="text-[10px] text-muted-foreground font-bold mt-0.5">{new Date(b.createdAt).toLocaleDateString()}</span>
                      </div>
                    </TableCell>
                    <TableCell className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div className="h-10 w-10 rounded-xl bg-primary/10 flex items-center justify-center text-primary font-black border border-primary/20 shadow-inner">
                          {(b.riderDetails?.name || b.user?.name || "?")[0]}
                        </div>
                        <div className="flex flex-col">
                          <span className="font-bold text-sm text-foreground">{b.riderDetails?.name || b.user?.name}</span>
                          <span className="text-[10px] text-muted-foreground font-medium">{b.user?.email}</span>
                        </div>
                      </div>
                    </TableCell>
                    <TableCell className="px-6 py-4">
                      <div className="flex flex-col">
                        <span className="font-black text-xs text-foreground uppercase tracking-wide">{b.bike?.name}</span>
                    ...
                    <TableCell className="px-6 py-4">
                      <div className="flex flex-col">
                        <span className="font-black text-sm text-foreground">₹{b.pricing?.totalAmount?.toLocaleString()}</span>
                        <Badge className={cn("w-fit mt-1 text-[8px] font-black uppercase px-2 py-0.5 rounded-md border", PAYMENT_CONFIG[b.payment?.status || "pending"].bg, PAYMENT_CONFIG[b.payment?.status || "pending"].text, PAYMENT_CONFIG[b.payment?.status || "pending"].border)}>
                          {PAYMENT_CONFIG[b.payment?.status || "pending"].label}
                        </Badge>
                      </div>
                    </TableCell>
                    <TableCell className="px-6 py-4">
                      <Select
                        value={b.status}
                        onValueChange={(val) => doAction(b._id, "update", { status: val })}
                        disabled={!!actionLoading}
                      >
                        <SelectTrigger className={cn("h-10 w-full min-w-[140px] rounded-xl font-black text-[10px] uppercase tracking-widest border-2 transition-all shadow-sm", STATUS_CONFIG[b.status]?.bg || "bg-muted", STATUS_CONFIG[b.status]?.text || "text-muted-foreground", STATUS_CONFIG[b.status]?.border || "border-border")}>
                          <div className="flex items-center gap-2">
                            {actionLoading === `update${b._id}` ? <Loader2 className="h-3 w-3 animate-spin" /> : <div className="h-1.5 w-1.5 rounded-full bg-current" />}
                            <SelectValue />
                          </div>
                        </SelectTrigger>
                        <SelectContent className="bg-card border-border rounded-xl shadow-2xl">
              ...
              <div className="bg-muted/30 p-8 border-b border-border flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 shrink-0">
                ...
                <div className="flex gap-4 shrink-0">
                  <div className="flex flex-col items-end gap-1">
                    ...
                    <Badge className={cn("px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest border shadow-sm", STATUS_CONFIG[selected.status]?.bg || "bg-muted", STATUS_CONFIG[selected.status]?.text || "text-muted-foreground", STATUS_CONFIG[selected.status]?.border || "border-border")}>
                      {STATUS_CONFIG[selected.status]?.label || selected.status}
                    </Badge>
                  </div>
                  <div className="flex flex-col items-end gap-1">
                    <span className="text-[8px] font-black text-muted-foreground uppercase tracking-widest">Payment</span>
                    <Badge className={cn("px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest border shadow-sm", PAYMENT_CONFIG[selected.payment?.status || "pending"].bg, PAYMENT_CONFIG[selected.payment?.status || "pending"].text, PAYMENT_CONFIG[selected.payment?.status || "pending"].border)}>
                      {PAYMENT_CONFIG[selected.payment?.status || "pending"].label}
                    </Badge>
                  </div>
                </div>
              </div>
                  ...
                  <div className="space-y-8">
                    <div className="space-y-4">
                      ...
                      <div className="p-6 rounded-[2rem] bg-muted/20 border border-border shadow-sm flex items-start gap-5 group hover:border-primary/30 transition-all">
                        <div className="h-16 w-16 rounded-2xl bg-primary/10 flex items-center justify-center text-primary font-black text-2xl border border-primary/20 shadow-inner group-hover:scale-105 transition-transform shrink-0">
                          {(selected.riderDetails?.name || selected.user?.name || "?")[0]}
                        </div>
                        <div className="space-y-1 flex-1 min-w-0">
                          <p className="font-black text-xl text-foreground truncate">{selected.riderDetails?.name || selected.user?.name}</p>
                          <div className="space-y-1.5 pt-2">
                            <p className="text-xs font-bold text-muted-foreground flex items-center gap-2.5">
                              <Phone className="h-3.5 w-3.5 text-primary/60" /> {selected.riderDetails?.phone || selected.user?.phone}
                            </p>
                            <p className="text-xs font-bold text-muted-foreground flex items-center gap-2.5">
                              <Mail className="h-3.5 w-3.5 text-primary/60" /> {selected.user?.email}
                            </p>
                            <p className="text-[10px] font-bold text-muted-foreground/60 leading-relaxed pt-2 border-t border-border/40 mt-2 line-clamp-2">
                              {selected.riderDetails?.address || "Address Not Provided"}
                            </p>
                          </div>
                        </div>
                      </div>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        {[
                          { label: "Driving License", url: selected.drivingLicense || selected.licenceUrl || selected.licenseUrl },
                          { label: "ID Proof", url: selected.idProof || selected.aadhaarUrl || selected.idProofUrl },
                          { label: "Rider Selfie", url: selected.selfieImage || selected.selfieUrl || selected.additionalDocUrl },
                          { label: "Payment Proof", url: selected.paymentProof || selected.paymentProofUrl },
                        ].map((doc) => {
                          const hasUrl = !!doc.url && doc.url !== "null";
                          const fullUrl = hasUrl ? (doc.url.startsWith("http") ? doc.url : `${API_URL}${doc.url.startsWith("/") ? "" : "/"}${doc.url}`) : null;
                      ...
                      <div className="p-6 rounded-[2.5rem] bg-card border border-border shadow-elegant relative overflow-hidden group hover:border-primary/20 transition-all">
                        <div className="flex items-center gap-6 relative z-10">
                          <div className="h-24 w-32 rounded-2xl overflow-hidden border border-border bg-muted/30 shadow-inner group-hover:scale-105 transition-transform shrink-0">
                            <img
                              src={selected.bike?.imageUrl?.startsWith("/uploads") ? `${API_URL}${selected.bike?.imageUrl}` : selected.bike?.imageUrl || "/placeholder-bike.jpg"}
                          ...
                          <div className="space-y-1 min-w-0">
                            ...
                            <p className="text-xl font-black text-foreground leading-none truncate">{selected.bike?.brand} {selected.bike?.model}</p>
                            <p className="text-xs font-bold text-muted-foreground mt-1">₹{selected.bike?.pricePerDay}/day Base Rate</p>
                          </div>
                        </div>
                      ...
                      <div className="p-8 rounded-[2.5rem] bg-gradient-brand text-primary-foreground shadow-glow relative overflow-hidden">
                        ...
                        <div className="space-y-3 relative z-10">
                          <div className="flex justify-between items-center text-xs opacity-80 font-bold border-b border-white/10 pb-2">
                            ...
                            <span>₹{selected.pricing?.basePrice?.toLocaleString()}</span>
                          </div>
                          <div className="flex justify-between items-center text-xs opacity-80 font-bold border-b border-white/10 pb-2">
                            <span>Security Deposit</span>
                            <span>₹{selected.pricing?.securityDeposit?.toLocaleString()}</span>
                          </div>
                          <div className="flex justify-between items-center text-xs opacity-80 font-bold border-b border-white/10 pb-2">
                            <span>Service & GST</span>
                            <span>₹{((selected.pricing?.gst || 0) + (selected.pricing?.platformFee || 0))?.toLocaleString()}</span>
                          </div>
                          <div className="flex justify-between items-center pt-4">
                            <span className="text-sm font-black uppercase tracking-widest">Gross Total</span>
                            <span className="text-3xl font-black">₹{selected.pricing?.totalAmount?.toLocaleString()}</span>
                          </div>
                        </div>
                      </div>
                    </div>
                ...
                <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                  ...
                  <div className="flex items-center gap-3">
                    <Button
                      ...
                      onClick={() => doAction(selected._id, "update", { status: "rejected" })}
                      disabled={!!actionLoading}
                    >
                      <X className="h-4 w-4" /> Flag Violation
                    </Button>
                    <Button
                      ...
                      onClick={() => doAction(selected._id, "update", { status: "confirmed", documentVerificationStatus: "verified" })}
                      disabled={!!actionLoading}
                    >
