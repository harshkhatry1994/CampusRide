    ...
    >
      <div className="relative aspect-[4/3] overflow-hidden bg-muted">
        ...
        <div className="absolute bottom-3 right-3 glass rounded-full px-2.5 py-1 text-xs flex items-center gap-1">
          <Star className="h-3 w-3 fill-primary text-primary" /> {bike.rating || 4.8}
        </div>
      </div>
      <div className="p-4">
        <div className="flex items-start justify-between gap-2">
          <div>
            <h3 className="font-semibold tracking-tight truncate max-w-[140px]">{bike.bike_name || `${bike.brand} ${bike.model}`}</h3>
        ...
        <div className="mt-4 pt-4 border-t border-border/40 flex items-center justify-between text-[10px] text-muted-foreground font-medium uppercase tracking-wider">
          <div className="flex items-center gap-1.5">
            <Fuel className="h-3 w-3 text-primary/70" /> {bike.fuel_type || "Petrol"}
          </div>
          <div className="flex items-center gap-1.5">
